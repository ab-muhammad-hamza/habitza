	...
	persist(
		...
		}),
		{
			...
			migrate: (persistedState, version) => {
				...
				for (let i = version; i < CURRENT_VERSION; i++) {
					...
					if (migration) {
						...
						try {
							...
							console.error(`Migration v${i} → v${i + 1} failed:`, e);
						}
					}
				}

				const habits = (newState as any)?.habits ?? [];
				const safeHabits = habits.map((h: any) => ({
					...h,
				...
				return { ...newState as any, habits: safeHabits };
			},
